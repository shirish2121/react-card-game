...
function App() {
  ...
  const [gameStarted, setGameStarted] = useState(stored.gameStarted || false);
  const [gameOver, setGameOver] = useState(stored.gameOver || false);
  const [winnerMessage, setWinnerMessage] = useState(
    stored.winnerMessage || ""
  );
  const [currentTrick, setCurrentTrick] = useState(stored.currentTrick || []);
  const [leadSuit, setLeadSuit] = useState(stored.leadSuit || null);
  const [awaitingTrump, setAwaitingTrump] = useState(
    stored.awaitingTrump || false
  );
  const [partialHands, setPartialHands] = useState(stored.partialHands || null);
  ...
  const handlePlayerCardClick = async (index) => {
    if (leadPlayerId !== 0 || currentTrick.some((t) => t.playerId === 0))
      return;
  ...
  const evaluateTrickAndContinue = async (trick, updated) => {
    ...
    if (uniquePlayers.size !== 4) {
      console.warn(
        "Skipping trick evaluation: duplicate or missing players",
        trick
      );
      debugger;
      return;
    ...
    if (isGameOver) {
      ...
      const newCaller =
        yourTeam === 13 || opponentTeam === 13
          ? (trumpCallerIndex + 2) % 4
          : updated.reduce(
              (max, p, i) => (p.tricksWon > updated[max].tricksWon ? i : max),
              0
            );
  ...
  const getCardPosition = (playerId) => {
    switch (playerId) {
      ...
      case 1:
        return { left: "15%", top: "50%", transform: "translate(50%, -50%)" }; // Bot 1
      case 2:
        return { top: "15%", left: "50%", transform: "translate(-50%, 30%)" }; // Bot 2
      case 3:
        return { top: "50%", right: "15%", transform: "translate(-50%, -50%)" }; // Bot 3
      default:
        return {};
    }
  };
      ...
      {!gameStarted ? (
        ...
        <>
          <div className="status-bar">
            ...
            <span className="status caller" title="Trump Caller">
              🎯 {players[trumpCallerIndex]?.name}
            </span>
            <span className="status lead" title="Lead Player">
              🕹️ {players[leadPlayerId]?.name}
            </span>
          </div>
          ...
          <div className="table-layout">
            ...
            <div className="bot bot-top" style={{ gridArea: "top" }}>
              ...
              <BotHand count={players[2]?.hand.length} />
            </div>
            ...
            <div className="trick-area">
              ...
              <div
                ...
                style={{
                  color: ["♥", "♦"].includes(trump) ? "crimson" : "black",
                }}
              >
                <span>{trump}</span>
              </div>
              ...
              <div className="hand-container">
                {players[0]?.hand?.length > 0 &&
                  (() => {
                    ...
                    const topHand = fullHand.slice(
                      0,
                      Math.max(0, fullHand.length - baseSize)
                    );
